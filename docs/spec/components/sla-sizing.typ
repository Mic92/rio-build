...
- `ioseq` (\~#qty("5", "s")): `O_DIRECT|O_DSYNC` 1 MiB sequential writes to `/var/rio/overlays` — NVMe RAID0 or gp3 quota volume per `nodeClassRef`. `O_DIRECT` defeats page cache; target size #qty("256", "MiB") looped to stay under `ephemeral-storage`.
...
#warning(title: [Untrusted-path gaps closed in §Phasing 13a])[
  Three sandbox-escape paths reach beyond per-tenant scope at `901829a3` and are *closed in 13a*, not the model:
