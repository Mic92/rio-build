...
if render_karpenter --set karpenter.quotaVolumeSize=1.5Ti >/dev/null 2>"$err"; then
  echo "FAIL: quotaVolumeSize=1.5Ti rendered (should fail — sprig int64 would coerce to 0)" >&2
  exit 1
fi
grep -q "quantityBytes" "$err" || {
  echo "FAIL: quotaVolumeSize=1.5Ti error does not name quantityBytes:" >&2
  sed 's/^/  /' "$err" >&2
  exit 1
}

# Negative: decimal-SI suffix (G, not Gi). Same int64→0 hazard.
if render_karpenter --set karpenter.quotaVolumeSize=500G >/dev/null 2>"$err"; then
  echo "FAIL: quotaVolumeSize=500G rendered (should fail — decimal SI is not supported)" >&2
  exit 1
fi
grep -q "quantityBytes" "$err" || {
  echo "FAIL: quotaVolumeSize=500G error does not name quantityBytes:" >&2
  sed 's/^/  /' "$err" >&2
  exit 1
}
...
got=$(render_controller_toml --set karpenter.quotaVolumeSize=500Gi | toml_int_key max_node_disk)
# 500 Gi × 0.9 reserve (controller.yaml) = 483183820800.
test "$got" -eq 483183820800 || {
  echo "FAIL: quotaVolumeSize=500Gi → max_node_disk=$got, expected 483183820800 (500Gi × 0.9)" >&2
  exit 1
}
...
got=$(render_controller_toml --set karpenter.quotaVolumeSize=2Ti | toml_int_key max_node_disk)
# 2 Ti × 0.9 = 1979120929996.8 → int64 truncates to 1979120929996.
test "$got" -eq 1979120929996 || {
  echo "FAIL: quotaVolumeSize=2Ti → max_node_disk=$got, expected 1979120929996 (2Ti × 0.9, int64-truncated)" >&2
  exit 1
}
