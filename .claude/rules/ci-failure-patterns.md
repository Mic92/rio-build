...
| **flannel subnet race** | `loadFlannelSubnetEnv failed: open /run/flannel/subnet.env` early in boot | Gated since eea0192d (`k3s-full.nix:720`). The log line is now a benign blip that recovers in <1s; if a subtest still times out, look past the flannel error for the real cause. |
